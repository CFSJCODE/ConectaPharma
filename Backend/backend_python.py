...
class Settings:
    ...
    CORS_ORIGINS: List[str] = [
        ...
        "https://conectapharma-33fd7.firebaseapp.com",
        "https://cfsjcode.github.io",
        "null",
    ]
    EXTRA_CORS_ORIGINS: List[str] = [
        origin.strip()
        for origin in os.getenv("CONNECTAPHARMA_CORS_ORIGINS", "").split(",")
        if origin.strip()
    ]
...
class RouteLinkService:
    @staticmethod
    def build(latitude: float, longitude: float, label: str = "Destino") -> Dict[str, str]:
        """Gera link gratuito de navegação baseado exclusivamente no OpenStreetMap."""
    ...
    async def fetch_from_overpass(cls, lat: float, lng: float, radius_km: float) -> List[Dict[str, Any]]:
        ...
        async with lock:
            ...
            for endpoint in endpoints:
                try:
                    response = await cls.get_client().post(endpoint, data={"data": query})
                    ...
                    last_error = exc
                    logger.warning("Falha no endpoint Overpass %s: %s", endpoint, exc)
        ...
        for record in records:
            ...
            items.append(
                FarmaciaProximaItem(
                    ...
                    maps_url=record.get("maps_url") or RouteLinkService.build(float(record["latitude"]), float(record["longitude"]), record.get("name") or "Farmácia")["maps_url"],
    ...
    async def search_nearby(
        ...
        for record in records:
            ...
            items.append(EstabelecimentoSaudeItem(
                ...
                opening_hours_raw=OpeningHoursService.raw(record),
                address_quality=record.get("address_quality", "osm_structured_or_fallback"),
                maps_url=record.get("maps_url") or RouteLinkService.build(float(record["latitude"]), float(record["longitude"]), record.get("name") or "Estabelecimento de saúde")["maps_url"],
...
async def listar_farmacias(
    ...
    for farmacia in db.farmacias:
        ...
        items.append(FarmaciaDiretorioItem(
            ...
            latitude=lat,
            longitude=lng,
            maps_url=RouteLinkService.build(float(lat), float(lng), farmacia.get("nome", "Farmácia"))["maps_url"] if lat is not None and lng is not None else None,
        ))
    return items[:limit]
...
async def listar_estabelecimentos_saude_proximos(
    ...
    Usa somente OpenStreetMap/Overpass como fonte externa gratuita.
...
async def listar_farmacias_proximas(
    ...
    source: str = Query("overpass", pattern="^(overpass|mock)$", description="Fonte de dados: overpass. Alternativa: mock."),
):
    ...
    a resposta. A API usa OpenStreetMap/Overpass como fonte externa única, com fallback local
    quando a consulta pública estiver indisponível. O backend aplica cache, calcula distância, avalia horário de funcionamento
